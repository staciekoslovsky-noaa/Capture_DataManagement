...
next_id <- RPostgreSQL::dbGetQuery(
  con,
  "SELECT max(id) FROM capture.tbl_sample_results_food_prey"
)
...
# Read CSV file
data <- read.csv(
  "C:\\Users\\Stacie.Hardy\\Work\\SMK\\Projects\\Capture\\Data\\CaptureSampleResults\\PEP Pv2015 hard parts 20260401.csv",
  header = TRUE
) %>%
  ...
  mutate(
    part_code = ifelse(bone == 'Whole/partial remains', 9999, part_code)
  ) %>%
  mutate(bone_side_lku = ifelse(bone_side_lku == "", NA, bone_side_lku)) %>%
  mutate(
    bone_condition_lku = ifelse(
      bone_condition_lku == "Good",
      "G",
      ifelse(
        bone_condition_lku == "Fair",
        "F",
  ...
  mutate(
    date_identified = ifelse(date_identified == "", '2/4/2026', date_identified)
  ) %>%
...
data_num_oto <- add_cols(data_num_oto, c('L', 'R', 'UNK'))
data_num_oto <- data_num_oto %>%
  replace_na(list(L = 0, R = 0, UNK = 0)) %>%
  rename(num_oto_left = L, num_oto_right = R, num_oto_unk = UNK) %>%
  ungroup()
...
data_num_beak <- data %>%
  ...
  count(bone_side_lku) %>%
  pivot_wider(names_from = bone_side_lku, values_from = n)
data_num_beak <- add_cols(data_num_beak, c('UP', 'LO'))
data_num_beak <- data_num_beak %>%
  replace_na(list(UP = 0, LO = 0)) %>%
  rename(num_beak_upper = UP, num_beak_lower = LO) %>%
  ungroup()
...
data_num_condition <- add_cols(data_num_condition, c('G', 'F', 'P'))
data_num_condition <- data_num_condition %>%
  rename(num_good = G, num_fair = F, num_poor = P) %>%
  ungroup() %>%
...
RPostgreSQL::dbWriteTable(
  con,
  c("capture", "tbl_sample_results_food_prey"),
  import_prey,
  append = TRUE,
  row.names = FALSE
)
RPostgreSQL::dbWriteTable(
  con,
  c("capture", "tbl_sample_results_food_bones"),
  import_bone,
  append = TRUE,
  row.names = FALSE
)
RPostgreSQL::dbWriteTable(
  con,
  c("capture", "tbl_sample_results_food_measure"),
  import_measure,
  append = TRUE,
  row.names = FALSE
)
